...
App.controller(
  "BookView",
  function ($scope, $q, $timeout, $state, $stateParams, Restangular) {
    // add the book in common library
    $scope.addBookLibrary = function () {
      $state.transitionTo("add-book", { id: $stateParams.id });
    };
    /**
     * Delete the book.
     */
    $scope.deleteBook = function () {
      if (confirm("Do you really want to delete this book?")) {
        Restangular.one("book", $stateParams.id)
          .remove()
          .then(function () {
    ...
    // Load tags
    var tagsPromise = Restangular.one("tag/list")
      .get()
      .then(function (data) {
    ...
    // Load book
    var bookPromise = Restangular.one("book", $stateParams.id)
      .get()
      .then(function (data) {
        $scope.book = data;
      });
